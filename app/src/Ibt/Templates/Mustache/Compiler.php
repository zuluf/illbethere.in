...
class Mustache_Compiler
{
    ...
    const VARIABLE_CALL = '
        $value .= $this->resolveValue($context->%s("%s"), $context, $indent, "%s");%s
        $buffer .= $value;
    ';
    /**
    ...
    private function variable($id, $filters, $escape, $level)
    {
        ...
        if ( count ( explode(' ', $id) ) <= 1) {
            ...
            return sprintf($this->prepare(self::VARIABLE_CALL, $level), 'find', $callable, $values, $filters, $this->flushIndent(), $value);
        }
    }
